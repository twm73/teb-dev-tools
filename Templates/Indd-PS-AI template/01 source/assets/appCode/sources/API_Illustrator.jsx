/**
 * Adobe Illustrator ExtendScript Utility
 *
 * The $._ext_ILST object contains utility methods for Adobe Illustrator ExtendScript development.
 *
 * @namespace $._ext_ILST
 */
$._ext_ILST={
    ...
    runList : function (config){
        ...
        for (var i = 0; i < appConfig.selectedElements.length; i++) {
            if(appConfig.selectedElements[i].name ==='toolsPanelNo2List1') addTextFrame(appConfig.selectedElements[i].text);
        }
    ...
    addTestTextInIllustrator : function (config){
        var appConfig;
        if(arguments.length >0){appConfig = config;}else appConfig = this.getAppConfig();
        addTextFrame('Hello Word', 20, 20);
        return JSON.stringify(config);
    },
    ...
    flattenAllLayers : function (config){
        var appConfig;
        if(arguments.length >0){appConfig = config;}else appConfig = this.getAppConfig();
        flattenAllLayers();
        return JSON.stringify(config);
